...
- **供应商类型筛选**：Routes 增加 Provider Type 筛选。选中某一类型后，只保留该类型的路由，Provider 芯片也只留下该类型的账号。配置路由、Playground 和 Request Logs 里选择供应商时，选项为「类型 · 别名」，同一类型排在一起。Providers 卡片按类型再按别名排列。路由卡片、拓扑和可靠性页仍显示「别名 · 类型」。
