...
export function tryParseUsageSummary(
	text: string,
	protocol: string
): string | null {
	...
	try {
		...
		if (u && typeof u === "object") {
			...
			const duration = u.duration ?? u.seconds;
			const bits: string[] = [];
			...
			// DashScope ASR 按音频秒数返回 usage.duration / usage.seconds。
			if (typeof duration === "number") bits.push(`duration: ${duration}s`);
...
export function extractUsageFromStreamChunk(
	...
	if (
		proto === "gemini" &&
		o.usageMetadata &&
		typeof o.usageMetadata === "object"
	) {
		return tryParseUsageSummary(
			JSON.stringify({ usageMetadata: o.usageMetadata }),
			"gemini"
		);
	}
	...
	if (typeof pt === "number" || typeof ct === "number") {
		...
		else if (typeof pt === "number" && typeof ct === "number")
...
export function parseLastStreamUsage(
	sseText: string,
	protocol: string
): string | null {
