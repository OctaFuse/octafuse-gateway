...
type ProviderAddCardProps = {
	onImport: () => void;
	onCreate: () => void;
};

export function ProviderAddCard(props: ProviderAddCardProps) {
	const { onImport, onCreate } = props;
	const t = useTranslations('providers.addCard');
		...
		<article className="flex h-full min-h-[8.5rem] flex-col justify-between rounded-xl border-2 border-dashed border-blue-200 bg-blue-50/50 p-4 shadow-sm">
			...
			<div className="mt-3 grid grid-cols-2 gap-2">
				<button
					type="button"
					onClick={onImport}
					className="inline-flex items-center justify-center gap-1.5 rounded-lg border border-blue-200 bg-white px-2.5 py-2 text-xs font-medium text-blue-800 hover:bg-blue-50 focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-500"
				>
