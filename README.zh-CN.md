...
本仓采用 **单一 semver 版本线**（根 `octafuse` 与 `@octafuse/*` 共用 `version`）。生产发版以 Git 标签 **`vX.Y.Z`** 为真源，触发 GHCR 镜像构建并在 GitHub Release 中记录各镜像 **digest**。

- 开发：在需要记入变更日志的 PR 中运行 **`npx changeset`** 并提交 **`.changeset/*.md`**
- 维护者：完整流程见 **[docs/ops/release-versioning.md](./docs/ops/release-versioning.md)** 与 **[`.changeset/README.md`](./.changeset/README.md)**
