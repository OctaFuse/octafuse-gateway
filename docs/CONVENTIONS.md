...
> 目标：在不立即拆出 `octafuse-docs` 子仓的前提下，让本仓既保留与代码强耦合的契约文档，又能逐步识别出可外移的“品牌/客户化”内容；同时杜绝把任何运行环境的真实密钥、Webhook、连接串写进 Git。
...
不放入本仓；适合放到独立的 `octafuse-docs` 站点或外部 wiki。本仓内只保留入口或最小说明，避免污染开源仓。
...
- 当 L3 候选项足够多、或开始有外部撰稿/翻译协作时，再讨论拆出独立 `octafuse-docs` 仓库；本文表格即为迁移清单初稿。
