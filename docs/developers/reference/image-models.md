...
- OpenAI：`packages/admin/lib/model-presets/openai.json`
- 字节 / 豆包：`packages/admin/lib/model-presets/bytedance.json`
...
- 旧 id（如 `doubao-seedream-4-5-*`）已从预设移除；库里若仍有旧行需手工清理。

其它 Provider 模板（智谱 / Together / xAI / OpenRouter 等）也可能带 `images.generations` URL，但 **当前静态 model-presets 仅上表三个 Image 模型**。新增厂商时：补 preset + Provider 模板 + 本文表格。
...
**Seedream**（官方按张；上游常只回 `usage.output_tokens≈16384`）：
...
仅配置 **`image_output_price`**；`image_input_*` 为 null。折算：

```text
image_output_price = 官方元/张 ÷ (典型_output_tokens / 1_000_000)
```

实测 `output_tokens` 偏离 16384 时，按上式重算并 PATCH。常量：`SEEDREAM_TYPICAL_OUTPUT_TOKENS`（`packages/core/src/db/image-token-usage.ts`）。
