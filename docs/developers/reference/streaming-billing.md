...
- 扣费：`status !== 'error'` 且 `charged_cost > 0`。金额公式：目录 `models.pricing_profile` 按 `input_tokens` 选档后，再乘模型官方时段倍率得到官方当刻价（`standard_cost`）；路由侧用户计费 / 供应成本 = 官方当刻价 × 路由有效倍率。无路由 `schedule.mode`（存量）时有效倍率 = 基础倍率 × 命中窗 factor（未命中为 1）；`mode: "override"` 时命中窗用窗口 factor，未命中用基础倍率。若 `users.charged_cost_factors` 含该目录模型 ID，再按 `system_config.USER_CHARGED_COST_FACTOR_MODE` 与路由 Charged 有效倍率合成最终 `charged_cost`（六位四舍五入）：默认 **`multiply`** 为路由用户计费 × 用户倍率；**`min`** 取两者较小值。供应成本与官方当刻价不变；未命中用户键不改金额。模型与路由分时时段都在请求进入 Gateway 时锁定（业务时区下的 `HH:mm` 与可选 ISO 星期），长流式请求跨越边界不会切换倍率；该时刻写入 `pricing_audit` 的目录时段与路由时段（`evaluated_at_utc`、`local_weekday`）。嵌套 `price_override.metered` / `charged` tiers **忽略**。`pricing_audit` 新写入为 v5（可带 `catalog_schedule`、`user_charged_factor`、`user_charged_factor_mode`、`combined_charged_factor`；用户未命中时 factor 为 `null`；v4 历史行仍可解析）。上线官方分时之前的日志 `standard_cost` 仍是裸目录价，不做回补。详见 `packages/proxy/src/services/usage-tracker.ts` 与 `packages/core/src/db/pricing-schedule.ts`。
