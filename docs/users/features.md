...
| 音频（Audio） | OpenAI 兼容 `/v1/audio/transcriptions` 与 `/v1/audio/speech`，DashScope 同步多模态 ASR，以及原生实时音频；ASR 支持按时长或 Token 计费，TTS 支持按字符计费。调试台验证实时 ASR 时可直接使用浏览器麦克风；默认 `GET /v1/models` 不含纯音频模型。`kind=audio` 仅返回语音转写；语音合成需 `kind=all`。 |
...
| 公开 Catalog | `GET /catalog/models` 无需用户 Key，聚合可用模型、协议能力和各路由组当前及后续时段价格，适合门户展示。用户个性化折扣走管理接口 `GET /admin/users/:id/display-discounts`（不计入用户 RPM）。需鉴权的 `GET /v1/models` 默认返回 LLM 和 `default,free` 路由组（不含文生图、ASR、TTS），供智能体使用。 |
...
| 预置供应商 / 模型 | 管理后台可从静态目录一键导入：除官方模型厂外，还覆盖聚合平台与各类 Coding / Token Plan；预填 Base URL 与模型目录价等信息，导入后补齐真实 API Key 并挂路由即可使用。当前预设还包括 `hy4-preview`、`qwen3.8-flash`、`glm-5.3-flash`、`glm-5.3-flashx`。模型导入不会自动写入模型标签，标签由管理员在导入后维护。完整清单见官网 [Providers Catalog](https://octafuse.dev/zh/catalog/providers/) 与 [Models Catalog](https://octafuse.dev/zh/catalog/models/)；Coding / Token Plan 的专用 endpoint 不应与普通按量模板混用。 |
