...
| 预算与计费 | 按用户 Key 记录请求、Token、成本与扣费。用户可同时拥有按日 / 周 / 月重置的**周期额度**和不会随周期重置的**永久额度**；扣费先使用周期额度，不足部分再扣永久额度。模型与智能体工具均区分 **供应成本（Metered）**、**官方当刻目录价（Standard）**、**用户计费（Charged）** 三笔账（工具在 catalog 直接配绝对单价，无 Route factor/schedule）。模型可配官方分时时段；路由倍率叠在官方当刻价之上，并可在**分时时段（Schedule）**里按可选星期覆盖倍率；用户详情还可按目录模型设置用户倍率，在路由用户计费之后继续打折、加价或免单。 |
