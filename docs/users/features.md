...
| 公开 Catalog | `GET /catalog/models` 无需用户 Key，聚合可用模型、协议能力和各路由组当前及后续时段价格，适合门户展示。用户个性化折扣走管理接口 `GET /admin/users/:id/display-discounts`（不计入用户 RPM）。需鉴权的 `GET /v1/models` 默认返回 LLM 和 `default,free` 路由组，供 Agent 使用。 |
