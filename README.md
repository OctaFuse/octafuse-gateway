...
**English:** [README.en.md](./README.en.md) · **官网介绍：** [octafuse.dev/zh](https://octafuse.dev/zh/)
...
以下步骤仅用于**本地开发**。生产或预发部署见 **[部署](#部署)**。先 clone 仓库，并在仓库根目录执行后续命令：
...
Docker quickstart 与 Cloudflare 本地 D1 路径都不需要先复制 `.env.example`。只有运行 Node + Postgres/MySQL 脚本，或自定义部署配置时，才需要使用 `.env` / `.env.local`。
